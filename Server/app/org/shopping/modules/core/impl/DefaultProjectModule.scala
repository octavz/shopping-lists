...
class DefaultProjectModule @Inject()(dalUser: UserDAL, dalProject: ListDAL) extends ListModule {
  ...
  override def getUserLists(id: String, offset: Int, count: Int): Result[ProjectListDTO] = {
    ...
    } yield resultSync(ProjectListDTO(items = res._1.map(p => new ListDTO(p)).distinct, total = res._2))
